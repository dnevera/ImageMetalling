...
namespace histogram{
    ...
    inline float4 histogramSampledColor(
                                        texture2d<float, access::sample>  inTexture,
                                        constant IMP::cropRegion          &regionIn,
                                        float                    scale,
                                        uint2 gid){
...
inline uint4 channel_binIndex(
    ...
    uint   Y       = uint(dot(inColor.rgb,IMP::Y_YCbCr_factor) * inColor.a * Im.x);
...
kernel void kernel_impHistogramRGBYCounter(
                                           ...
                                           device   IMP::histogramBuffer     &out       [[ buffer(0)]],
                                           constant IMP::cropRegion          &regionIn  [[ buffer(1)]],
...
//    float4 inColor = histogram::histogramSampledColor(inTexture,regionIn,scale,gid);
//    uint   Y   = uint(dot(inColor.rgb,IMP::Y_YCbCr_factor) * inColor.a * Im.x);
    uint4  rgby = channel_binIndex(inTexture,regionIn,scale,gid);// (uint3(inColor.rgb * Im), Y);
...
kernel void kernel_impPartialRGBYHistogram(
                                           texture2d<float, access::sample>       inTexture  [[texture(0)]],
                                           device   IMP::histogramPartialBuffer  *outArray   [[ buffer(0)]],
                                           constant IMP::cropRegion               &regionIn  [[ buffer(1)]],
                                           constant float                         &scale     [[ buffer(2)]],
                                           //
