...
namespace imetalling {
    ...
    id<MTLComputePipelineState> make_pipeline(id<MTLDevice>& device, const std::string& kernel_name) {
        ...
        if (libpath.empty()){
            ...
            {
                log::error(true, " *** Function::make_pipeline error: new default library cannot be created...");
                return nil;
            }
        }
        else
        ...
        {
            log::error(true, " *** Function::make_pipeline error: new library %s cannot be created...", libpath.c_str());
            return nil;
        }

        if (!(kernelFunction  = [metalLibrary newFunctionWithName:[NSString stringWithUTF8String:kernel_name.c_str()]]))
        {
            log::error(true, " *** Function::make_pipeline error: new kernel function %s cannot be created for %s lib...", kernel_name.c_str(), libpath.c_str());
            [metalLibrary DEHANCER_RELEASE];
            return nil;
        }
        ...
        {
            log::error(true, " *** Function::make_pipeline error: new pipeline state cannot be created for %...", kernel_name.c_str());
            [metalLibrary DEHANCER_RELEASE];
            [kernelFunction DEHANCER_RELEASE];
            return nil;
        }
    ...
    Texture Function::make_texture(const void *command_queue, size_t width, size_t height, size_t depth) {
        ...
        if (height == 1 && depth == 1){
            ...
            auto t = [queue.device newTextureWithDescriptor:descriptor];
...
#else
    Function::Function(void *command_queue, const std::string& kernel_name):
            command_queue_(command_queue),
            kernel_name_(kernel_name)
    {
